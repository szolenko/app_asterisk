		...
		if ($class_rec['ID'])
		  {
			...
			if ($object_rec['ID'])
			  {
				$method_rec=SQLSelectOne("SELECT * FROM methods WHERE OBJECT_ID='".$object_rec['ID']."' AND TITLE = 'Events'");
				if ($method_rec['ID'])
				  {
					if (stripos($method_rec['CODE'], $data['Event']) == false)
					  {
// New event. Add function to method Events in class Asterisk
						echo date('Y-m-d H:i:s')." : Receive new event. Add function ".$data['Event']."\n";
						$method_rec['CODE'] .= "function ".$data['Event'].'($option)'."\n";
						$method_rec['CODE'] .= "	{\n";
						$method_rec['CODE'] .= "		/**\n";
...
// Event already exist. Process event
  						  $method = $object_rec['TITLE'].".Events";
